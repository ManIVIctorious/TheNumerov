...
{ 
  ...
  */
  ...
  spline (count, 0, 0, 0.0, 0.0, x, y, b, c, d, &iflag);
...
{  /* begin procedure spline() */
  ...
  *iflag = 0;
  ...
  for (i=0; i < n; i++)
  {
    printf("\n   %d   %12.8lf    %12.8lf    %12.8lf", i, b[i], c[i], d[i]);
  ...
  */
  ...
  { 
    ...
    n_interval = round ((x[i+1] - x[i])/dx);

    // printf("Debug  %25.18lf    %25.18lf     %d\n", x[i+1] - x[i], dx, n_interval);

    for (j = 0; j < n_interval; j++)
     {
       r = x[i] + (double) j * dx;

       y_spline = seval(n, r, x, y, b, c, d, &last);
       printf("%20.14lf   %20.14lf\n",r,y_spline);
     }
  }

  // output last entry in spline-func
  r = x[i];
  y_spline = seval(n, r, x, y, b, c, d, &last);
  printf("%20.14lf   %20.14lf\n",r,y_spline);
  ...
  */
  ...
  if (factor >= 0.0)
  {
    ...
    r_1 = (-2.0 * c[min_idx] + factor)/6.0/d[min_idx];
  ...
  if (factor >= 0.0)
  {
    ...
    r_1 = (-2.0 * c[min_idx+1] + factor)/6.0/d[min_idx+1];
